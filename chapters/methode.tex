...
Doch egal welches Medium benutzt wird, jeder Übertragungskanal\index{Übertragungskanal} verändert das Signal durch seine Umwelteinflüsse während es ihn durchläuft. In der Luft etwa kommen die übrigen Geräusche der Umgebung hinzu, u.a. auch die Eigenreflexion der Schallwelle an der Umgebung. 
...
Ein Synchronisations-Code\index{Synchronisations-Code} besteht aus $L_s$ Bit, daher sind ebenso viele Frames notwendig um ein $sync$ zu schreiben. Diese Gruppe an Frames wird als \textit{Sync Block} bezeichnet.
...
Somit lässt sich der Übermittelungsprozess eines Watermarks konzeptionell beschreiben als eine Partitionierung des Watermarks in Messages geeigneter Länge (bedingt durch das Verhältnis $L_m$ zu $L_c$ des ECC\index{Fehlerkorrekturverfahren}) und deren Versand durch Pakete.
...
Wie man sich anhand der Abbildung \ref{fig:protocol} leicht überlegen kann, können immer nachdem ein Codeword erfolgreich gelesen wurde die folgenden $L_c$ Samples Sections \index{Sample Section} auf der Suche nach weiteren $sync$ Sequenzen übersprungen werden.
